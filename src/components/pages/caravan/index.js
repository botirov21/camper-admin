...
export default function CaravanComponent() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFilteredData(caravans.data);
        setAllData(caravans.data);
      } catch (error) {
  ...
  const handleClick = async () => {
    try {
      const response = await fetch(`${BASEURL}/caravans/addNewCaravan`, {
        method: "POST",
        headers: {
  ...
  const handleUpdateClick = async (_id) => {
    try {
      const response = await fetch(`${BASEURL}/caravans/${_id}`, {
        method: "PUT",
        headers: {
  ...
  const handleDeleteClick = async (id) => {
    try {
      ...
      if (response.ok) {
        ...
        setCaravans(caravans.filter((caravan) => caravan.id !== id));
      }
    } catch (error) {
